...
이 저장소는 최적화 결과물만 추적합니다. 새 Hero 미디어를 만들거나 교체할 때만 아래 원본 파일을 로컬에 잠시 두고 `bun run prepare:hero-assets` 를 실행하세요.

각 폴더에서 아래 원본 파일명을 임시로 사용할 수 있습니다.
...
원본 파일들은 현재 `.gitignore` 로 제외되며, 최적화가 끝나면 삭제해도 됩니다.

## 생성 파일

`bun run prepare:hero-assets` 를 실행하면 Hero가 실제로 읽는 최적화 파일이 생성됩니다. public repo에는 아래 생성 파일만 남기는 것이 기본 원칙입니다.
...
2. 원본 파일명을 규칙에 맞춰 로컬에 넣습니다.
3. `bun run prepare:hero-assets` 를 실행합니다.
4. 필요하면 원본 PNG/JPG와 `video.mp4` 를 다시 삭제합니다.
5. `components/HeroDemoAssets.js` 에 팩 이름과 라벨만 추가/수정합니다.
